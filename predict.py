...
def create_midi(prediction_output):
    ...
    for pattern in prediction_output:
        parts = pattern.split()
        _instrument, _notes, _duration = (parts[0].replace('_', ' '), parts[1:-1], parts[-1])
        ...
        if (isinstance(_notes, list)):
            ...
            _chord = set_duration(_chord,_duration)
            instruments[_instrument].append(_chord)
        else:
            _note = note.Note(_notes)
            _note = set_duration(_note,_duration)
    ...
    for instrument_key in instruments.keys():
        ...
        try:
            current_instrument = instrument.fromString(instrument_key)
        except:
